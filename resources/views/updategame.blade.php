    ...
    <div class="container">
        ...
        <form action="/update/{{ $data->id }}" method="post" enctype="multipart/form-data">
            ...
            <div class="row mb-5" style="color:gray;">
                ...
                <div class="col-sm-10">
                    <input name="title" value="{{  $data->title }}"type="text" class="form-control @error('title') is-invalid @enderror" id="title"
                        autofocus>
                    @error('title')
            ...
            <div class="row mb-5" style="color:gray;">
                ...
                {{-- <div id="preview-image" class="" style="margin-bottom: 0rem;"> --}}
                    <img id="profile-image" src="{{ Storage::url($data->image) }}"class='col-6 col-sm-1 rounded-circle'
            ...
            <div class="row mb-5" style="color:gray;">
                ...
                <div class="col-sm-10">
                    <textarea name="desc" type="text" rows="4" cols="50"class="form-control @error('desc') is-invalid @enderror" id="desc">{{ $data->desc }}</textarea>
                    @error('desc')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
            ...
            <div id="NewGgenre" class="row mb-5" style="color:gray;visibility:hidden;">
                ...
                <div class="col-sm-10">
                    <input name="newGenre" type="text" class="form-control @error('newGenre') is-invalid @enderror"
                        id="newGenre">
                    @error('newGenre')
                    <script>
            ...
            <div class="row mb-5" style="color:gray;">
                ...
                <div class="col-sm-10">
                    <select name="PEGI_rating" id="inputRating" class="form-select">
                        {{-- <option selected>{{ $data->PEGI_rating}}</option> --}}
                        @if($data->PEGI_rating!='0')
                        <option>0</option>
                        @else
                        <option selected>{{ $data->PEGI_rating}}</option>
                        @endif
                        @if($data->PEGI_rating!='3')
                        <option>3</option>
                        @else
                        <option selected>{{ $data->PEGI_rating}}</option>
                        @endif
                        @if($data->PEGI_rating!='7')
                        <option>7</option>
                        @else
                        <option selected>{{ $data->PEGI_rating}}</option>
                        @endif
                        @if($data->PEGI_rating!='12')
                        <option>12</option>
                        @else
                        <option selected>{{ $data->PEGI_rating}}</option>
                        @endif
                        @if($data->PEGI_rating!='16')
                        <option>16</option>
                        @else
                        <option selected>{{ $data->PEGI_rating}}</option>
                        @endif
                        @if($data->PEGI_rating!='18')
                        <option>18</option>
                        @else
                        <option selected>{{ $data->PEGI_rating}}</option>
                        @endif
